	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "admin.php?c=venues&a=img_upload",  	// 上传文件的路径
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '	<div class="upload_box hid">';
					html += '		<div class="upload_main hid">';
					html += '			<div class="upload_choose hid">';
	            	html += '				<div class="convent_choice hid">';
	            	html += '					<div class="andArea hid">';
	            	html += '						<div class="filePicker hid">点击选择图片</div>';
					...
					html += '		</div>';
					html += '		<div id="uploadInf" class="upload_inf" style = "display:none;" ></div>';
					html += '	</div>';
					html += '</form>';
				}else{
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>上传成功，文件地址是：<span class = 'imgname'>" + response + "</span></p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
